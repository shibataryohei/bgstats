...
kruskal_table <- function(df, group, omit, digits){
  ...
  colnames(result)[2] <- paste0(colnames(result)[2], " (N = ", number_groups[1], ")")
  colnames(result)[3] <- paste0(colnames(result)[3], " (N = ", number_groups[2], ")")
  colnames(result)[4] <- paste0(colnames(result)[4], " (N = ", number_groups[3], ")")
  
  result
} 

fisher_table3 <- function(df, group, omit){
  df %>% 
    dplyr::select(where(is.factor)) %>% 
    mutate_all(droplevels) -> df1
  ...
  lapply(1:ncol(df2), function(i){
    ...
    mutate(Variable2 = paste0(Number2, " (",
                              sprintf(Ratio2*100, fmt = '%#.1f'), "%)" )) %>% 
    mutate(Variable3 = paste0(Number3, " (",
                              sprintf(Ratio3*100, fmt = '%#.1f'), "%)" )) %>% 
    mutate(Variable = paste0(Variable, " [", Factor, "]")) %>% 
    dplyr::select(Variable, Variable1, Variable2, Variable3, P.value, SD) %>% 
    mutate_if(is.character, as.factor) -> result
  
  colnames(result)[2] <- paste0(levels[1], " (N = ", number_groups[1], ")")
  colnames(result)[3] <- paste0(levels[2], " (N = ", number_groups[2], ")")
  colnames(result)[4] <- paste0(levels[3], " (N = ", number_groups[3], ")")
...
kruskal_fisher_table <- function(df, group, omit, digits){
  ...
  kruskal_table(df,
               group = group,
               omit = NA,
               digits = digits) -> result_kruskal
  
  result_fisher %>% 
    bind_rows(result_kruskal) %>% 
    arrange(P.value) 
}
